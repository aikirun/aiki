		...
		completed: {
			request: (id: string, expectedRevision: number) =>
				({
					type: "optimistic",
					id,
					state: { status: "completed", output: { encodedValue: { receiptId: "child-receipt-9" } } },
					expectedRevision,
				}) as const,
			expectedChildState: { status: "completed", output: { encodedValue: { receiptId: "child-receipt-9" } } },
		},
		failed: {
			request: (id: string, expectedRevision: number) =>
				({
		...
		withHarness(async ({ context, repos, publisher }) => {
			...
			await withFakeClock(childTerminatedAt, () =>
				stateMachine.transitionState(context, {
					type: "optimistic",
					id: child.runId,
					state: { status: "completed", output: { encodedValue: { receiptId: "rcp-7" } } },
