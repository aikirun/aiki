...
- Type assertions are a last resort, with two accepted uses: branded ids and timestamps (`as WorkflowRunId`, `as TimestampMs`) and `as const`. Non-empty arrays are never asserted with `as NonEmptyArray<T>` and never assembled by hand from a destructured first element. When non-emptiness is already proven (a length-preserving `.map` over a `NonEmptyArray`, or a value just past an `isNonEmptyArray` guard), wrap it in `asNonEmptyArray(...)` from `@aikirun/lib/collection/array`; when it is not proven, use the `isNonEmptyArray` guard. Never assert to silence a genuine type error, and never use non-null assertions (`!`) without a proven invariant right above (see the testing guide for the one accepted pattern).
