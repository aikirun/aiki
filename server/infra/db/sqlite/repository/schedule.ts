	...
	return {
		...
		async bulkUpdateOccurrence(
			...
			for (const entry of entries) {
				ids.push(entry.id);
				nextRunAtCaseFragments.push(sql`WHEN ${entry.id} THEN ${entry.nextRunAt.toISOString()}`);
				if (entry.lastOccurrence) {
					lastOccurrenceCaseFragments.push(sql`WHEN ${entry.id} THEN ${entry.lastOccurrence.toISOString()}`);
				}
			}
