...
- Your application uses the Aiki SDK client to reach the server.
- Requests optionally pass through a pluggable Authorizer.
- The Aiki server orchestrates through a Handler (API routes, state machine) and
  a Runtime (crons, timer consumers, outbox drain) over one database, which is
  the source of truth for all state.
- The server delivers work two ways:
  - Push: a signed HTTP request to Endpoints (serverless, push-based).
  - Pull: long-lived Workers discover work through a subscriber — the server's
    claim API by default, or a pluggable message transport (Publisher and
    Subscriber, e.g. Redis).
- Workers and endpoints execute the workflow and store results back on the server.
```
