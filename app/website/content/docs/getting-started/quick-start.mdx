---
title: Quick Start
description: Run a workflow that suspends for two weeks waiting on a payment, and resumes where it left off - server, worker and all, in one process.
---
...
- **Task** — `activateTrial` / `downgradeToFree`: a unit of work whose result is persisted, so a restart never redoes it. → [Tasks](../core-concepts/tasks.md)
- **Workflow** — `trialV1`: orchestrates tasks and events, tracked at every step. → [Workflows](../core-concepts/workflows.md)
