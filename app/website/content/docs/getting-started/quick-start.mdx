---
title: Quick Start
description: Run a workflow that waits two weeks for a payment and resumes where it left off, all in one process.
---
...
```bash
DATABASE_URL=postgresql://user:password@your-db-host:5432/aiki \
  bunx aiki-server migrate apply
```
...
> **Tip:** by default `wait` never gives up. Pass `{ timeout: { seconds: 60 } }` to bound it.
...
- **[Task](../core-concepts/tasks.md)** — `activateTrial` / `downgradeToFree`: a unit of work whose result is persisted, so a restart never redoes it.
- **[Workflow](../core-concepts/workflows.md)** — `trialV1`: orchestrates tasks and events, tracked at every step.
- **[Event](../core-concepts/events.md)** — `paymentReceived`: `wait()` suspends with nothing held; `send()` or the 14-day timeout resumes it.
- **[Server](../architecture/server.md)** — `server({ db })` + `runtime.start()`: the orchestrator and its background loops.
- **[Client](../core-concepts/client.mdx)** — `client({ handler })`: the in-process connection; workers and your app both attach.
- **[Worker](../core-concepts/workers.md)** — `worker({...}).start(client)`: claims ready runs and executes them.
...
Beyond this demo, give the server a timer priority queue so sleeps and timeouts fire exactly when they come due rather than at the next database scan — see [Installation](./installation.md).
